...
#define JNI_CROP_FN(A) Java_org_ebookdroid_core_crop_ ## A

#define COLUMN_HALF_HEIGHT 12

// 扫描步进
#define H_LINE_SIZE 6

#define V_LINE_SIZE 8

//边距留白,与切割的图片大小是有关的,缩略图越小,留白应该越小,因为精度小
#define LINE_MARGIN 8

//这个值越小,表示忽略的空间越大,比如一行就一个页码,如果这个适中,就直接忽略,认为这行是空白的
#define WHITE_THRESHOLD 0.004
...
int
calculateAvgLum(uint8_t *src, int width, int height, int sub_x, int sub_y, int sub_w, int sub_h);
...
jobject
JNI_CROP_FN(PageCropper_nativeGetCropBounds)(JNIEnv *env, jclass clz,
                                             jobject pixels, jint width,
                                             jint height, jfloat left, jfloat top,
                                             jfloat right,
                                             jfloat bottom) {
    uint8_t *src;
    ...
    int avgLum;
    avgLum = 225;//calculateAvgLum(src, width, height, 0, 0, width, height);
...
jobject
JNI_CROP_FN(PageCropper_nativeGetColumn)(JNIEnv *env, jclass clz, jobject pixels,
                                         jint width,
    ...
    int avgLum;
    avgLum = 225;//calculateAvgLum(src, width, height, top, bottom, right - left, bottom - top);
...
float getLeftBound(uint8_t *src, int width, int height, int avgLum) {
    ...
    for (x = 0; x < w; x += V_LINE_SIZE) {
        ...
        int white = isRectWhite(src, width, height, x, LINE_MARGIN, V_LINE_SIZE,
                                height - 2 * LINE_MARGIN, avgLum);
        if (white) {
            whiteCount++;
        } else {
            if (whiteCount >= 1) {
...
float getTopBound(uint8_t *src, int width, int height, int avgLum) {
    ...
    for (y = 0; y < h; y += H_LINE_SIZE) {
        int white = isRectWhite(src, width, height, LINE_MARGIN, y, width - 2 * LINE_MARGIN,
                                H_LINE_SIZE, avgLum);
        if (white) {
            whiteCount++;
        } else {
...
int isRectWhite(uint8_t *src, int width, int height, int sub_x, int sub_y, int sub_w, int sub_h,
    ...
    for (y = 0; y < sub_h; y++) {
        for (x = 0; x < sub_w; x++) {
            ...
            if ((lum < avgLum) && ((avgLum - lum) * 9 > avgLum)) {
                count++;
            }
        }
    }
