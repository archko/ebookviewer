...
int CharacterHelper_init(CharacterHelper *that, JNIEnv *env) {
    ...
    if (that->cls) {
        that->midToLowerCase = (*(that->jenv))->GetStaticMethodID(that->jenv, that->cls,
                                                                  "toLowerCase", "(C)C");
    }
...
unsigned short CharacterHelper_toLowerCase(CharacterHelper *that, unsigned short ch) {
    return that->valid ? (*(that->jenv))->CallStaticCharMethod(that->jenv, that->cls,
                                                               that->midToLowerCase, ch) : ch;
}
...
int ArrayListHelper_init(ArrayListHelper *that, JNIEnv *env) {
    ...
    if (that->cls) {
        ...
        that->midAdd = (*(that->jenv))->GetMethodID(that->jenv, that->cls, "add",
                                                    "(Ljava/lang/Object;)Z");
    }
...
int PageTextBoxHelper_init(PageTextBoxHelper *that, JNIEnv *env) {
    ...
    if (that->cls) {
        ...
        that->fidText = (*(that->jenv))->GetFieldID(that->jenv, that->cls, "text",
                                                    "Ljava/lang/String;");
    }

    that->valid = that->cls && that->cid && that->fidLeft && that->fidTop && that->fidRight &&
                  that->fidBottom && that->fidText;
    return that->valid;
}
...
jobject PageTextBoxHelper_setRect(PageTextBoxHelper *that, jobject ptb, const int *coords) {
    if (that->valid && ptb) {
        (*(that->jenv))->SetFloatField(that->jenv, ptb, that->fidLeft, (jfloat)(
        float) coords[0]);
        (*(that->jenv))->SetFloatField(that->jenv, ptb, that->fidTop, (jfloat)(
        float) coords[1]);
        (*(that->jenv))->SetFloatField(that->jenv, ptb, that->fidRight, (jfloat)(
        float) coords[2]);
        (*(that->jenv))->SetFloatField(that->jenv, ptb, that->fidBottom, (jfloat)(
        float) coords[3]);
    }
    return ptb;
}
