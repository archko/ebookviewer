...
abstract class AbstractNotificationManager implements INotificationManager {
    ...
    protected PendingIntent getIntent(final Intent intent) {
        return intent != null ? PendingIntent.getActivity(EBookDroidApp.context, 0, intent, PendingIntent.FLAG_IMMUTABLE) : getDefaultIntent();
    }

    protected PendingIntent getDefaultIntent() {
        return PendingIntent.getActivity(BaseDroidApp.context, 0, new Intent(), PendingIntent.FLAG_IMMUTABLE);
    }
