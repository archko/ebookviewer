...
public class GLES20Canvas implements GLCanvas {
    ...
    public GLES20Canvas() {
        ...
        mBoxCoordinates = uploadBuffer(boxBuffer, FLOAT_SIZE);
    ...
    public void drawRect(float x, float y, float width, float height, Paint paint) {
        draw(GLES20.GL_LINE_LOOP, OFFSET_DRAW_RECT, COUNT_RECT_VERTEX, x, y, width, height, paint.getColor(), paint.getStrokeWidth());
        mCountDrawLine++;
    }
    ...
    public void deleteRecycledResources() {
        synchronized (mUnboundTextures) {
            ...
            if (mUnboundTextures.size() > 0) {
                GLES20IdImpl.glDeleteTextures(null, ids.size(), ids.getInternalArray(), 0);
                ids.clear();
            }

            ids = mDeleteBuffers;
            if (ids.size() > 0) {
                GLES20IdImpl.glDeleteBuffers(null, ids.size(), ids.getInternalArray(), 0);
                ids.clear();
            }
        }
    }
    ...
    @Override
    public void setClipPath(PointF... path) {
    ...
    @Override
    public void drawPoly(int color, PointF... path) {

    }

    @Override
    public void fillPoly(int color, PointF... path) {
    ...
    private int uploadBuffer(Buffer buffer, int elementSize) {
        GLES20IdImpl.glGenBuffers(1, mTempIntArray, 0);
        checkError();
